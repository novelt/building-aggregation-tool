...
def gdm_process(start_step, end_step, args):
    ...
    step_list = [
        ...
        steps.step_adjust_ref_raster_size,

        steps.fix_reproject_split_buildings,

        steps.step_group_buildings,

        steps.step_fill_center_building_groups,

        steps.step_buffer_buildings,

        steps.step_union_all_buffers,

        steps.step_create_centroids,

        steps.step_create_new_database,

        steps.create_building_count_raster,
        steps.step_contour_lines,
        steps.step_contour_poly_stats,

        steps.step_grid_slice_settlements,
        steps.step_split_grid_slice_settlements,

        #steps.step_import_settlement_slices_to_db,

        steps.step_include_raster_centers,

        steps.step_create_corner_shapes,
        steps.step_union_center_shapes,

        steps.create_settlement_table,

        # Categorize settlements
        steps.step_area_bldg_count,
        steps.step_make_buas_from_contours,

        steps.step_export_settlements_to_fgb,
        # We want to check that the settlements cover all squares
        # so building count == total # of buildings in all settlements

        steps.step_grid_slice_settlements_final,
        steps.step_split_grid_slice_settlements_final,

        steps.step_intersect_buildings,
        steps.import_buildings_to_db,

        steps.check_building_counts,
    ]
